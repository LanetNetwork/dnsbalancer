...
struct rb_table *rb_create(rb_comparison_func * compare, void *param,
...
const void *rb_delete(struct rb_table *tree, const void *item)
{
	struct rb_node *pa[RB_MAX_HEIGHT];	/* Nodes on stack. */
	unsigned char da[RB_MAX_HEIGHT];	/* Directions moved from stack nodes. */
	int k;			/* Stack height. */
	...
	if (p->rb_link[1] == NULL)
		...
		if (r->rb_link[0] == NULL) {
			...
			t = (enum rb_color)r->rb_color;
			r->rb_color = p->rb_color;
			p->rb_color = (unsigned char)t;
...
static void trav_refresh(struct rb_traverser *trav)
{
	...
	if (trav->rb_node != NULL) {
		...
		for (i = trav->rb_table->rb_root; i != node;) {
			...
			i = i->rb_link[cmp(node->rb_data, i->rb_data, param) >
				       0];
		}
	}
}
...
struct rb_table *rb_copy(const struct rb_table *org, rb_copy_func * copy,
			 ...
			 struct libavl_allocator *allocator)
{
	...
	for (;;) {
		while (x->rb_link[0] != NULL) {
			...
			memcpy(&stack[height++], &x,
			       sizeof(const struct rb_node *));
			stack[height++] = y;
			x = x->rb_link[0];
			y = y->rb_link[0];
		}
...
const void *(rb_assert_delete) (struct rb_table * table, const void *item) {
	const void *p = rb_delete(table, item);
	assert(p != NULL);
	return p;
}
